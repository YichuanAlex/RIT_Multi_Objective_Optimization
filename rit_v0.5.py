#!/usr/bin/env python3
# End-to-end MLA-ROBD with refined grid + plotting fixes (hidden=256, layers=2)
# Main fixes: map MLA post-hoc losses back to original units (scale_sq) so bars are visible;
# robustness improvements for plotting and saving.
...
def run_grid_search_and_final(args):
    ...
    # refined grid
    lambda1_list = args.lambda1_list if args.lambda1_list is not None else [1.0]
    lambda2_list = args.lambda2_list if args.lambda2_list is not None else [0.0]
    lambda3_list = args.lambda3_list if args.lambda3_list is not None else [3.0]
    mu_list = args.mu_list if args.mu_list is not None else [0.3]
