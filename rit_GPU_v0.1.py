#!/usr/bin/env python3
# End-to-end MLA-ROBD with:
#  - combo dir names including main training params
#  - skip combos if results already exist
#  - CUDA acceleration (if available)
#  - plotting fixes
#  - robust saving of logs/metrics/models
...
class LSTMAction(nn.Module):
    ...
    def forward_once(self, y_seq: torch.Tensor, x_prev: torch.Tensor) -> torch.Tensor:
        ...
        # Convert to shape (1, t, 1)
        feat = y_seq.unsqueeze(0).unsqueeze(-1).contiguous()
        _, (h_T, _) = self.lstm(feat)
        h = h_T[-1, 0, :]  # last layer hidden, shape (hidden,)
        h = self.dropout(h)
        # x_prev is scalar tensor (device consistent)
        x_prev_ = x_prev.unsqueeze(0)  # shape (1,)
        hcat = torch.cat([h, x_prev_], dim=-1)  # shape (hidden+1)
        out = self.fc(hcat)  # shape (1,)
        return out.squeeze()  # scalar tensor
...
class E2ETrainer:
    ...
    def forward_window(self, y_seq: torch.Tensor, x0: float = 0.0) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for t in range(1, T + 1):
            y_hist = y_seq[:t]
            y_t = y_seq[t - 1]
            x_ml = self.base.forward_once(y_hist, prev_cal)  # scalar tensor
...
def save_objective_hist_box(out_dir: Path, robd_losses, ml_losses, e2e_losses):
    ...
    plt.figure()
    # use tick_labels to avoid deprecation warning where available
    try:
        plt.boxplot([robd_losses, ml_losses, e2e_losses], tick_labels=["R-OBD", "ML-only", "E2E"])
    except TypeError:
        # older matplotlib
        plt.boxplot([robd_losses, ml_losses, e2e_losses])
        plt.xticks([1, 2, 3], ["R-OBD", "ML-only", "E2E"])
...
def run_grid_search_and_final(args):
    ...
    # refined grid (can be overridden by args)
    lambda1_list = args.lambda1_list if args.lambda1_list is not None else [0.5, 1.0, 2.0, 3.0]
    lambda2_list = args.lambda2_list if args.lambda2_list is not None else [0.5, 1.0, 2.0, 3.0]  # often zero in experiments
    ...
    for (l1, l2, l3, mu) in combos:
        combo_idx += 1
        # include main training params in folder name
        combo_name = (f"l1={l1}_l2={l2}_l3={l3}_mu={mu}_hidden={args.hidden}_"
                      f"layers={args.layers}_dropout={args.dropout}_lr={args.lr}_seed={args.seed}")
        combo_dir = Path(args.out) / combo_name
        combo_dir.mkdir(parents=True, exist_ok=True)

        # skip if we already have val_metrics.json AND model_state.pt (assume combo done)
        val_metrics_file = combo_dir / "val_metrics.json"
        model_state_file = combo_dir / "model_state.pt"
        if val_metrics_file.exists() and model_state_file.exists():
            print(f"[{combo_idx}/{total}] SKIP combo {combo_name} (existing results found).")
            # attempt to load val metrics for summary
            try:
                with open(val_metrics_file, "r", encoding="utf-8") as f:
                    val_metrics = json.load(f)
                metrics = {
                    "val_post_avg": float(val_metrics.get("val_post_avg", np.nan)),
                    "val_pre_avg": float(val_metrics.get("val_pre_avg", np.nan)),
                    "best_epoch": int(val_metrics.get("best_epoch", -1)),
                    "epochs_ran": int(val_metrics.get("epochs_ran", args.tune_epochs))
                }
            except Exception:
                metrics = {"val_post_avg": float('inf'), "val_pre_avg": float('inf'), "best_epoch": -1, "epochs_ran": args.tune_epochs}
            # load model state if possible
            trained_base = LSTMAction(hidden_size=args.hidden, num_layers=args.layers, dropout_rate=args.dropout).to(device)
            try:
                sd = torch.load(model_state_file, map_location=device)
                trained_base.load_state_dict(sd)
            except Exception:
                trained_base = None
            train_log = []
            runtime = 0.0
            ...
            # save model state (cpu copy to keep files portable)
            try:
                cpu_state = {k: v.cpu() for k, v in trained_base.state_dict().items()}
                torch.save(cpu_state, combo_dir / "model_state.pt")
            except Exception as e:
                print("Warning: failed to save model state:", e)

        # add to summary rows
        summary_rows.append({
            "lambda1": float(l1), "lambda2": float(l2), "lambda3": float(l3), "mu": float(mu),
            "val_post_avg": float(metrics.get("val_post_avg", float('inf'))),
            "val_pre_avg": float(metrics.get("val_pre_avg", float('inf'))),
            "best_epoch": int(metrics.get("best_epoch", -1)),
            "epochs_ran": int(metrics.get("epochs_ran", args.tune_epochs)),
            "runtime": float(runtime),
            "combo_dir": str(combo_dir)
        })

        # update best
        if metrics.get("val_post_avg", float('inf')) < best_val:
            best_val = float(metrics.get("val_post_avg", float('inf')))
        ...
        print(f"  combo done. val_post_avg={metrics.get('val_post_avg', np.nan):.6f} (runtime {runtime:.1f}s)")
    ...
    if best_state is not None:
        try:
            ...
            # best_state might be None or incompatible; ignore and train from scratch
            pass
    ...
    final_dir = Path(args.out) / (f"best_final_l1={best_combo[0]}_l2={best_combo[1]}_l3={best_combo[2]}_mu={best_combo[3]}"
                                  f"_hidden={args.hidden}_layers={args.layers}_dropout={args.dropout}_lr={args.lr}_seed={args.seed}")
    final_dir.mkdir(parents=True, exist_ok=True)
    try:
        cpu_state = {k: v.cpu() for k, v in final_base.state_dict().items()}
        torch.save(cpu_state, final_dir / "final_model_state.pt")
    except Exception as e:
        print("Warning: failed to save final model state:", e)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tune", type=int, default=1, help="1 to run grid tuning (not used to skip combos in filesystem)")
