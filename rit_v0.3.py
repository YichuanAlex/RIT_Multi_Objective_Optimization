#!/usr/bin/env python3
# e2e_mla_robd_final.py
# End-to-end MLA-ROBD training + grid search + outputs including mla_tradeoff.csv/.png and objective hist/box.
# python rit_v0.3.py --csv AI_workload.csv --out outputs --epochs 80 --hidden 128 --tune 1
...
class LSTMAction(nn.Module):
    def __init__(self, hidden_size: int = 64, num_layers: int = 1, dropout_rate: float = 0.2):
        super().__init__()
...
def posthoc_mla_grid_analysis(ml_seqs: List[np.ndarray], windows: List[np.ndarray], m: float, theta_grid: List[float]):
    results = []
    for theta in theta_grid:
        ...
        for w, h in zip(windows, ml_seqs):
            ...
            for t in range(len(w)):
                ...
                prev = x_t
            losses.append(total / float(len(w)))
        results.append({"theta": float(theta), "avg_loss": float(np.mean(losses)), "worst_loss": float(np.max(losses))})
    return results
...
def run_grid_search_and_final(args):
    ...
    lambda1_list = args.lambda1_list if args.lambda1_list is not None else [0.1, 0.2, 0.5]
    lambda2_list = args.lambda2_list if args.lambda2_list is not None else [0.0]
    lambda3_list = args.lambda3_list if args.lambda3_list is not None else [1.0, 2.0, 3.0]
    mu_list = args.mu_list if args.mu_list is not None else [0.3, 0.5, 0.7]
    ...
    # Retrain best combo on full train_windows
    ...
    # Train ML-only baseline (full train)
    ...
    mla_results = posthoc_mla_grid_analysis(ml_seqs, test_windows, args.m, args.theta_grid)
    mla_df = pd.DataFrame(mla_results)
    mla_csv_out = Path(args.out) / "mla_tradeoff.csv"
    mla_df.to_csv(mla_csv_out, index=False)

    # Save plots to final_dir AND also to outputs root with exact names required
    # objective_hist
    plt.figure()
    ...
    hist_out = Path(args.out) / "objective_hist.png"
    plt.savefig(hist_out)
    plt.close()

    # objective_box
    plt.figure()
    # Matplotlib newer versions use 'labels' or 'tick_labels'; use labels arg
    plt.boxplot([robd_losses, ml_losses, e2e_losses], labels=["R-OBD", "ML-only", "E2E"])
    plt.title("Objective comparison")
    box_out = Path(args.out) / "objective_box.png"
    plt.savefig(box_out)
    plt.close()
    ...
    plt.legend()
    plt.tight_layout()
    mla_plot_out = Path(args.out) / "mla_tradeoff.png"
    plt.savefig(mla_plot_out)
    plt.close()

    # Also save same plots into final_dir for record
    import shutil
...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv", type=str, required=True, help="CSV, second column is used as series")
